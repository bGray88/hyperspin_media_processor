...
class FileProcessor():
    ...
    def __init__(self):
        self.logFile = LogFile.LogFile(self.getDataFile(filename='_Hyper_Log'), '_Hyper_Log')
        self.dbFile = Database.Database(self.getDataFile(filename='_Hyper_Data'), '_Hyper_Data')
    ...
    def writeDatabaseDetails(self, fileName, writable, version):
        sortList = []
        database = open(fileName, writable, encoding='utf-8')
